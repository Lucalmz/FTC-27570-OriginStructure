...
@Deprecated
public class ForwardAccelerationTuner extends OpMode {
    ...
    public static double MaxVel = FollowerConstants.xMovement;
    ...
    public void init() {
        ...
        telemetryA.addLine("The robot will run at 1 power until it reaches " + MaxVel + " inch/sec.");
    ...
    public void loop() {
        ...
        poseUpdater.update();
        if (!end) {
            if (Math.abs(poseUpdater.getVelocity().getXComponent()) > MaxVel) {
                end = true;
                for (DcMotorEx motor : motors) {
                    motor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
                    motor.setPower(0);
                ...
                accelerations.add(poseUpdater.getAcceleration().getXComponent());
            ...
            telemetryA.addData("forward acceleration:", average);
            telemetryA.update();
        }
    }
}
