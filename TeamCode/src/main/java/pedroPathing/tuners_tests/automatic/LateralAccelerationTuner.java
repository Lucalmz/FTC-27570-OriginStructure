...
@Deprecated
public class LateralAccelerationTuner extends OpMode {
    ...
    public static double MaxVel = FollowerConstants.yMovement;
    ...
    public void loop() {
        ...
        poseUpdater.update();
        if (!end) {
            if (Math.abs(poseUpdater.getVelocity().getYComponent()) > MaxVel) {
                end = true;
                for (DcMotorEx motor : motors) {
                    motor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
                    motor.setPower(0);
                ...
                accelerations.add(poseUpdater.getAcceleration().getYComponent());
            ...
            telemetryA.addData("lateral acceleration:", average);
            telemetryA.update();
        }
    }
}
